...
# تخصيص التصميم الاحترافي (تنسيق RTL، لون عام ذهبي فاتح، تكبير خط القائمة الجانبية وتلوين التبويب المختار)
st.markdown(
    """
    <style>
    ...
    /* تصميم القائمة الجانبية وتكبير الخط وتلوين التبويب المختار */
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, #d97706 0%, #b45309 50%, #92400e 100%);
        color: #ffffff;
    }
    
    /* تلوين أزرار الراديو في القائمة الجانبية وتكبير خطها وتغيير لون العنصر المختار */
    ...
    /* تخصيص مميز للتبويب (Radio Option) المُختار حالياً بلون مختلف متناسق (أزرق داكن أنيق أو أحمر داكن يبرز بوضوح) */
    [data-testid="stSidebar"] div[data-baseweb="radio"] input:checked + div {
        background-color: #1e3a8a !important; /* لون أزرق مميز عند الاختيار */
        border-radius: 8px;
    }
    ...
    /* تخصيص الأزرار داخل الشريط الجانبي */
...
st.markdown(
    """
    <div style="background: linear-gradient(135deg, #f59e0b, #d97706); padding: 20px; border-radius: 12px; color: white; text-align: center; margin-bottom: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
...
# --- جعل الأزرار والإحصائيات الرئيسية ثابتة في جميع الصفحات ---
st.markdown(
    "<p style='color: gray; font-size: 13px; margin-bottom: 5px;'>لوحة المؤشرات السريعة (اضغط على أي بطاقة للانتقال الفوري للتقرير):</p>",
    unsafe_allow_html=True,
)
...
with dcol1:
    ...
    if st.button("تقرير الموظفين", key="btn_emp_global"):
...
with dcol2:
    ...
    if st.button("تقرير الأجهزة", key="btn_pc_global"):
...
with dcol3:
    ...
    if st.button("تقرير الشاشات", key="btn_mon_global"):
...
with dcol4:
    ...
    if st.button("تقرير الطابعات", key="btn_prn_global"):
...
with dcol5:
    ...
    if st.button("تقرير الأعطال", key="btn_flt_global"):
            ...
            try:
                ...
                if st.button("معالجة وإضافة البيانات المرفوعة"):
                    st.session_state.assets_df = pd.concat(
                        [st.session_state.assets_df, df_imported],
                        ignore_index=True,
                    )
                    st.success(
            ...
            if not employee or not department:
                ...
                st.session_state.assets_df = pd.concat(
                    [
                        ...
                        pd.DataFrame(
                            [new_row],
                            columns=st.session_state.assets_df.columns,
                        ),
                    ],
                    ignore_index=True,
                )
...
# --- 5. تفاصيل إحصائيات الشاشات ---
    ...
    if total_records > 0:
        ...
        if len(mon_df) > 0:
            mon_counts = (
                mon_df["مقاس/نوع الشاشة"].value_counts().reset_index()
            )
...
# --- 7. تفاصيل الأعطال التقنية والصيانة ---
    ...
    st.subheader(
        "⚠️ متابعة الأعطال التقنية وتحديث حالة الأجهزة إلى (عمل / سليم)"
    )
    if total_records > 0:
        faults_df = df[df["حالة العطل"] != "سليم"]
        st.metric(
            "إجمالي الأجهزة التي بها أعطال أو قيد الصيانة", len(faults_df)
        )
        ...
        if len(faults_df) > 0:
            ...
            st.markdown("---")
            st.markdown(
                "#### 🛠️ تحديث حالة جهاز معطل وإرجاعه إلى (يعمل / سليم)"
            )
            ...
            if st.button("تحديث الحالة إلى (يعمل وجاهز)"):
                if selected_serial_to_fix:
                    ...
                    st.success(
                        f"تم تحديث الجهاز ذو السيريال ({selected_serial_to_fix}) إلى حالة (سليم ويعمل) بنجاح!"
                    )
                    st.rerun()
                else:
            ...
            st.success(
                "ممتاز! لا توجد أي أعطال مسجلة حالياً، كافة الأجهزة سليمة وتعمل بكفاءة."
            )
    else:
        st.info("لا توجد بيانات مسجلة.")
        ...
        with st.form("new_user_form"):
            ...
            if add_user_btn:
                if not new_username or not new_password:
                    ...
                    new_username
                    in st.session_state.users_df["اسم المستخدم"].values
                ):
                    ...
                    st.session_state.users_df = pd.concat(
                        [st.session_state.users_df, new_u_row],
                        ignore_index=True,
                    )
        ...
        if selected_target_user:
            ...
            with st.form("edit_user_form"):
                ...
                with ecol3:
                    ...
                    curr_role_idx = (
                        current_role_options.index(
                            current_user_data["الصلاحية"]
                        )
                ...
                update_user_btn = st.form_submit_button(
                    "حفظ التعديلات على المستخدم"
                )
                if update_user_btn:
                    idx = st.session_state.users_df[
                        st.session_state.users_df["اسم المستخدم"]
                        == selected_target_user
                    ].index[0]
                    if new_pass_edit.strip() != "":
                        st.session_state.users_df.at[
                            idx, "كلمة المرور"
                        ] = new_pass_edit
                    st.session_state.users_df.at[
                        idx, "الحالة"
                    ...
                    st.success(
                        f"تم تحديث بيانات المستخدم ({selected_target_user}) بنجاح!"
                    )
                    st.rerun()
